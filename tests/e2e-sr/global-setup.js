...
module.exports = async () => {
  ...
  // WordPress can interrupt the redirect with the "Confirm your administration
  // email" screen, which lives on wp-login.php - so waiting for a /wp-admin/
  // URL would time out. Go to the admin directly and check we arrived logged in.
  if (!page.url().startsWith(`${baseURL}/wp-admin/`)) {
    await page.goto(`${baseURL}/wp-admin/`);
  }
  if (new URL(page.url()).origin !== expectedOrigin) {
    await browser.close();
    throw new Error(`Landed on a different origin after login (${page.url()}); expected ${expectedOrigin}.`);
  }
  if (await page.locator('#loginform').count()) {
    await browser.close();
    throw new Error('Login did not take: still on the login form. Check WP_USERNAME / WP_PASSWORD.');
  }
